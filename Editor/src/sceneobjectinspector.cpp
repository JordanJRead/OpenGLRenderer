...
void SceneObjectInspector::toggleSelect(SceneObject* sceneObject) {
    ...
    if (mSelectedObjectViewer.get()
        && mSelectedObjectViewer.get() == sceneObject) {
        mSelectedObjectViewer.lookAt(nullptr);
    } else {
...
void SceneObjectInspector::updateRender() {
    if (mSelectedObjectViewer.get()) {
        ...
        for (auto componentIt{ components.begin() };
        ...
        ImGui::SameLine();
        if (ImGui::Button("Add")) {
            mSelectedObjectViewer.get()->addComponentFromName(
              mComponentDropdown.getCurrentType()); // TODO rename
        }
    }
...
SceneObject* SceneObjectInspector::getSceneObject() {
    return mSelectedObjectViewer.get();
}
