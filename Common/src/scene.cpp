...
void Scene::render(const ShaderMesh& meshShader, const ShaderPointLight& pointLightShader, const Framebuffer* const framebuffer, const RenderSettings& renderSettings, SceneObject* selectedObject) const {
	glDisable(GL_BLEND);
	renderSceneObject(*mRootObject, meshShader, pointLightShader, framebuffer, renderSettings, selectedObject);
}

void Scene::renderSceneObject(const SceneObject& sceneObject, const ShaderMesh& meshShader, const ShaderPointLight& pointLightShader, const Framebuffer* const framebuffer, const RenderSettings& renderSettings, const SceneObject* selectedObject) const {
	...
	for (const auto& child : sceneObject.getChildren()) {
		renderSceneObject(*child, meshShader, pointLightShader, framebuffer, renderSettings, selectedObject);
	}
}
...
void Scene::getPointLightData(const SceneObject& sceneObject, std::vector<float>& data) {
	const PointLight* pointLight{ sceneObject.getComponent<PointLight>() };
	if (pointLight) {
